...
export async function seed() {
  ...
  service cloud.firestore {
    match /databases/{database}/documents {
      ...
      // Admins can read ALL challenges (including hidden ones, for admin console)
      // Players can only read visible challenges
      match /challenges/{id} {
      ...
      // ── Solves collection ────────────────────────────────
      // All authenticated users can read solves (needed for leaderboard/activity)
      // Only the solve owner can create their solve entry
      match /solves/{id} {
      ...
      // Users can write their own entry; admins can write any
  ...
  ══════════════════════════════════════════════════════════════
  IMPORTANT: After pasting, also create these Firestore indexes
  (Firebase Console → Firestore → Indexes → Composite → Add):
    Collection: solves  |  Fields: uid ASC, ts DESC
    Collection: users   |  Fields: status ASC, joinedAt DESC
  Or just use the simple rules above — the app code uses
  client-side filtering as fallback so indexes are NOT required.
  ══════════════════════════════════════════════════════════════
  */
